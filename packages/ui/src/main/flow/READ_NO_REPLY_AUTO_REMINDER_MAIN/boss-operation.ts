...
5. 【动作规范与联系方式处理 - 引导在平台沟通】：
   ...
   - 【邮件通知仅限复杂决策】：仅当 HR 提出超出 Agent 知识范围的复杂考核要求、预约具体线下笔面试时间等必须求职者本人亲自决策的事项时，才调用 \`send_email_to_candidate\`。
6. 普通对话请直接回复短句。
...
export const getEmailForwardToolDefinition = (customReplyMessage?: string) => ({
  ...
  function: {
    ...
    description:
      '当HR提出超出当前Agent能力范围的要求（如布置Demo/上机实操题、索要特定数据、预约具体线下笔面试时间等需要求职者本人亲自决策的事项）时调用此工具向求职者发送邮件通知。注意：索要微信/电话请直接回复文本引导在平台沟通，严禁调用此工具。',
    parameters: {
      type: 'object',
      properties: {
        reason: {
          type: 'string',
          description:
            '触发发邮件的具体原因或HR核心诉求摘要（例如：HR布置实操Demo题、HR预约线下到场面试时间等）'
        },
        reply_message: {
          type: 'string',
